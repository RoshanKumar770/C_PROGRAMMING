...
void readStrings(char str[], char pat[], char rep[]) {
    ...
    fgets(pat, MAX, stdin);
    pat[strlen(pat) - 1] = '\0';  // Remove the newline character

    printf("Enter the replace string (REP): ");
    fgets(rep, MAX, stdin);
    rep[strlen(rep) - 1] = '\0';  // Remove the newline character
}
...
int findPattern(char str[], char pat[], int start) {
    ...
    if (pat[j] == '\0') {
        return i - j;  // Return the starting position of the pattern
    } else {
...
void replaceAllOccurrences(char str[], char pat[], char rep[]) {
    ...
    while (str[i] != '\0') {
        ...
        if (pos == -1) {
            ...
            // Skip the pattern in the original string
            i += pat_len;
            found = 1;
        }
    }
...
int main() {
    ...
    // Replacing all occurrences of pattern with replace string
